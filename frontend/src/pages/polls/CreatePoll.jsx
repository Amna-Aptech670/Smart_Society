    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="bg-card border border-border rounded-xl p-6 max-w-lg flex flex-col gap-3">
        <div>
          ...
          <input value={question} onChange={(e) => setQuestion(e.target.value)} required className="w-full border border-input rounded-lg px-3 py-2 mt-1 bg-background" placeholder="Should we install a new gym?" />
        </div>
        <div>
          <label className="text-sm text-muted-foreground">Options</label>
          {options.map((opt, i) => (
            <input key={i} value={opt} onChange={(e) => updateOption(i, e.target.value)} className="w-full border border-input rounded-lg px-3 py-2 mt-1 bg-background" placeholder={`Option ${i + 1}`} />
          ))}
        ...
        <div>
          ...
          <input type="date" value={expiresAt} onChange={(e) => setExpiresAt(e.target.value)} className="w-full border border-input rounded-lg px-3 py-2 mt-1 bg-background" />
        </div>
